<div x-data="{open: false}" class="block w-full lg:w-1/3">
    <label class="relative z-20">
        <div x-on:focusin="open=true" x-on:keydown.esc="open = false" x-on:focusout="open=false">
            <a href="{{  route ('search', ['result' => $this->search]) }}">
            ...
            <input x-on:input="open = true" wire:model.debounce="search" type="search" class="ml-4 shadow-[0_20px_25px_-5px_rgba(0,0,0,0.1)] h-max placeholder:text-lg placeholder:text-gray-500 w-full bg-white rounded-l-lg py-3 pl-4 focus:outline-none text-sm" placeholder="busque aqui" name="search" autocomplete="off"/>
    ...
    <div x-cloak x-show="open" class="absolute z-10 bg-black opacity-40 w-full h-auto left-0 right-0 bottom-0 top-0"></div>
</div>
